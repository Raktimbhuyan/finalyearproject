...
const STRENGTH_COLORS = ['bg-white/10', 'bg-red-400', 'bg-amber-400', 'bg-indigo-400', 'bg-emerald-400']
...
const Signup = () => {
  ...
  const validate = () => {
    ...
    if (!values.acceptTerms) next.acceptTerms = 'You need to accept the terms to continue.'
    setErrors(next)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // await fetch('/api/auth/signup', { method: 'POST', body: JSON.stringify({ ...values, method }) })
  ...
  const score = getPasswordScore(values.password)
    ...
    <>
      ...
      <section className="relative min-h-screen flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md rounded-3xl border border-white/10 bg-white/6 backdrop-blur-2xl shadow-[0_30px_80px_-25px_rgba(52,55,160,0.6)] p-8 sm:p-10">
          ...
          <p className="mt-1 text-sm text-white/60">Let&apos;s get your account ready.</p>
          ...
          <form onSubmit={handleSubmit} noValidate className="mt-8 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {errors.name && <p role="alert" className="text-xs text-red-400">{errors.name}</p>}
            </div>
                ...
                <div className="mt-1 flex items-center gap-2">
                  <div className="flex flex-1 gap-1">
                    {[0, 1, 2, 3].map((i) => (
                      <span
                        key={i}
                        className={`h-1 flex-1 rounded-full ${i < score ? STRENGTH_COLORS[score] : 'bg-white/10'}`}
                      />
                    ))}
                  </div>
                  <span className="text-[11px] text-white/40">{STRENGTH_LABELS[score]}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {errors.confirmPassword && <p role="alert" className="text-xs text-red-400">{errors.confirmPassword}</p>}
            </div>

            {/* Terms */}
            <label className="flex cursor-pointer items-start gap-2.5 text-sm leading-snug text-white/60">
              <input
                type="checkbox"
                checked={values.acceptTerms}
                onChange={update('acceptTerms')}
                className="mt-0.5 h-4 w-4 shrink-0 rounded accent-indigo-400"
              />
              <span>
                I agree to the{' '}
                <Link href="/terms" className="font-medium text-white underline decoration-indigo-400 underline-offset-4">
                  Terms of Service
                </Link>{' '}
                and{' '}
                <Link href="/privacy" className="font-medium text-white underline decoration-indigo-400 underline-offset-4">
                  Privacy Policy
                </Link>
                .
              </span>
            </label>
            {errors.acceptTerms && <p role="alert" className="text-xs text-red-400">{errors.acceptTerms}</p>}
            ...
            >
              {submitting ? <SpinnerIcon className="h-[18px] w-[18px] animate-spin" /> : 'Create account'}
            </button>
...
function UserIcon(props) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...props}>
      ...
      <path d="M4.5 20c1.4-3.5 4-5.3 7.5-5.3s6.1 1.8 7.5 5.3" />
    </svg>
  )
}
